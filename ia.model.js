...
// ── Búsqueda difusa (último recurso) ──────────────────────────────
// Se usa cuando la búsqueda exacta (LIKE %termino%) no encontró nada:
// el cliente pudo escribir un sinónimo, un typo, o una palabra que solo
// aparece en la DESCRIPCIÓN del producto (no en el nombre). Busca por
// coincidencia fonética (SOUNDEX) en el nombre Y por texto libre en la
// descripción, dentro de la categoría si se conoce.
// ⚠️ Antes esta función no existía: product.service.js la llamaba pero
// nunca estaba definida, así que la búsqueda difusa siempre fallaba en
// silencio (el error quedaba atrapado en un try/catch) y el bot terminaba
// respondiendo "No encontré ese producto" aunque sí existiera.
exports.searchProductosFuzzy = async (query, categoria = null) => {
    if (!query) return [];
    ...
    if (categoria) {
        const [rows] = await db.query(
            `SELECT p.id_producto AS id, p.nombre, p.precio_venta AS precio,
             ...
             WHERE c.nombre = ?
               AND p.estado = 'ACTIVO'
               AND (SOUNDEX(p.nombre) = SOUNDEX(?) OR p.descripcion LIKE ?)
             ORDER BY p.stock_actual DESC
             LIMIT 6`,
