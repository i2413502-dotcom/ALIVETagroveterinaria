...
// ── Validación en vivo del correo (¿ya existe una cuenta con este
//    correo?) — distinta a la de arriba, que solo revisa formato/
//    dominio. Reutiliza el mismo <input id="correo"> y su propio
//    debounce, para no interferir con el otro chequeo. ──
if (correoInputEl) {
    const avisoYaRegistrado = document.getElementById('correo-ya-registrado');
    let debounceCorreoRegistrado;
    correoInputEl.addEventListener('input', () => {
        clearTimeout(debounceCorreoRegistrado);
        if (avisoYaRegistrado) avisoYaRegistrado.classList.add('d-none');
        debounceCorreoRegistrado = setTimeout(async () => {
            if (!correoInputEl.value.includes('@')) return;
            try {
                const res = await fetch(`/api/auth/correo-registrado?correo=${encodeURIComponent(correoInputEl.value.trim())}`);
                const data = await res.json();
                if (avisoYaRegistrado) avisoYaRegistrado.classList.toggle('d-none', !data.registrado);
            } catch {
                // Si falla la verificación, no se bloquea el registro —
                // solo no se muestra el aviso (el envío del formulario
                // igual falla del lado del servidor si el correo ya existe).
            }
        }, 600);
    });
}
